...
def main():
	...
	if os.name == 'nt':
		hostname = os.environ['COMPUTERNAME'].lower() + '.' + 'saber3d.ru'
		codepg = 'cp1251'
		...
		megaraid = cmd_line(smartctl_scan, codepg)
		for i in range(len(megaraid)):
			if megaraid[i] != '':
				drive_list.append(('/dev/bus/0 ' + megaraid[i]).split(' '))
				json_string.append({'{#DEVNAME}': megaraid[i].replace(',', '')})
	...
	for i in range(len(devices)):
		if devices[i] != '':
			drive_list.append((devices[i].split()[0] + drive_type).split(' '))
			json_string.append({'{#DEVNAME}': devices[i].replace(',', '').split()[0]})
	...
	for disk in drive_list:
		...
		if 'megaraid' in disk[1]:
			...
			metric += smart_parser(smart_out, disk[0].replace(',', ''), hostname)
	#print(metric)
